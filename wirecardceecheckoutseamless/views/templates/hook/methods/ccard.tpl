...
{if $current.payment->getPci3DssSaqAEnable()}
    ...
    <div id="pt_wirecardcheckoutseamless_{$current.name}_iframe" class="wcs_iframe"></div>
...
{else}
    ...
    {if $current.payment->getDisplaycardholder()}
        <div class="form-group">
            ...
            <input type="text" name="cardholder" autocomplete="off" class="form-control cardholder"
                   placeholder="{l s={$current.payment->getCardholderPlaceholderText()} mod='wirecardceecheckoutseamless'}"
    ...
    <div class="form-group">
        ...
        <input type="tel" name="cardnumber" autocomplete="off" class="form-control cardnumber is_required wcs-validate"
               placeholder="{l s={$current.payment->getPanPlaceholderText()} mod='wirecardceecheckoutseamless'}"
    ...
    <div class="form-group">
        ...
        <div class="row">
            <div class="col-xs-2">
                <select name="pt_wcs_{$current.name}_expirationMonth" class="form-control month is_required wcs-validate"
                        data-wcs-fieldname="expirationMonth">
                    {foreach $current.payment->getMonths() as $k => $v }
                        <option value="{$k}"{if 0} selected="selected"{/if}>{$v}</option>
                    {/foreach}
                </select>
            </div>
            <div class="col-xs-3">
                <select name="pt_wcs_{$current.name}_expirationYear" class="form-control year is_required wcs-validate"
    ...
    <div class="form-group">
        ...
        <input type="tel" name="cvc" autocomplete="off" class="form-control cvc"
               placeholder="{l s={$current.payment->getCvcPlaceholderText()} mod='wirecardceecheckoutseamless'}"
    ...
    {if $current.payment->getDisplayIssueDateField()}
        <div class="form-group">
            ...
            <div class="row">
                <div class="col-sm-1">
                    <select name="pt_wcs_{$current.name}_issueMonth" class="form-control month"
                ...
                <div class="col-sm-2">
                    <select name="pt_wcs_{$current.name}_issueYear" class="form-control year"
